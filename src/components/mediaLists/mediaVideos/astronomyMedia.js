import React from 'react';

class Astronomy extends React.Component {
  render () {
    return (
      <div>
        <a href='https://www.youtube.com/watch?v=FunVM44dy_c' target_blank
        >The Precession of the Equinoxes Explained with Graham Hancock</a><br />
        <a href='https://www.youtube.com/watch?v=AJCQANGJSCM' target_blank
        >The Great Year - Precession of the Equinoxes and Ancient Man - Celestial Cycles - Yuga Cycle</a><br />
        <a href='https://www.youtube.com/watch?v=qc7MfcKF1-s' target ='_blank'
        >Eclipses</a><br />
        <a href='https://www.youtube.com/watch?v=DD_8Jm5pTLk' target ='_blank'
        >Reasons for the seasons - Rebecca Kaplan</a><br />
        <a href='https://www.youtube.com/watch?v=8izJDenkdgU&t=451s'
        >Sirius the Black Sun</a><br />
        <a href='https://www.youtube.com/watch?v=IJhgZBn-LHg'
        >Earth's movement</a><br />
        <a href='https://www.youtube.com/watch?v=mxhxL1LzKww&t=1135s'
        >How much of the earth can you see at Once?</a><br />
        <a href='https://www.youtube.com/watch?v=L1bT2NJN-O8' target ='_blank'
        >Polaris: The North Star</a><br />
      </div>
    );
  }
}

export default Astronomy;
