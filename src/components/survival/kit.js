import React from 'react';

class Kit extends React.Component {
  render () {
    return (
      <div>
      <li>Comunications:</li>
      Phones, chargers, additional batteries, walkie Talkies, Ham Radio,
      Solar USB Charger, Headsets, Mirror, Paper, Pencils, code book<hr/>
      <li>Defense:</li>
      Guns, Bullets, machete, Binoculars, paracord,<hr/>
      <li>Fire Making:</li>
      Axe, Magnifying Glass, Lighters, Cotton/vaseline, flint/steel, matches,
      shovel<hr/>
      <li>First Aid:</li>
      Manual, Bandaids, Neosporine, Alcohol, peroxide, Sling, Gauze, Tape,
      Tweezers, Scisors, stitching glue, Fiber pills, needles , Alergy medicine,
      Pain Medicine, Imodium AD, sunblock, Insect Repelant, chapstick, Gold Bond,
      Duct tape, Finger splints, Eye drops, Eye Gunk, Anti Itch Cream, Sun Block,
      Razor, Prescriptions, Girl Stuff<hr/>
      <li>Food:</li>
      Guns, Bullets, Tang, Canned Chicken, Rice-aRoni, Tuna, Lentils, MRE's,
      Mac and Cheese, Powdered Milk, Instant Pudding, instant Jello, Mess Kit,
      Utensils, Wash Pad, dish soap, powered eggs, Sugar, Salt, Powdered Butter,
      Raman, Dehydrated fruit, Oatmeal, cooking spray, powdered Mashed potatoes,
      Fishing pole and gear<hr/>
      <li>Navigation:</li>
      GPS, Batteries, Maps, List of Locations, routes, Compass, Phone Numbers<hr/>
      <li>Recreation:</li>
      Cards, Phase 10, Dominoes, Uno, Bananagrams, MInichess/Checkers,
      Harmonica, Books, Scriptures,<hr/>
      <li>Shelter:</li>
      Tent, Sleeping Bags, Blankets, Tarps, Rain Gear, Tape, Bungee Cords, Rope,
      stakes, Emergency Blankets, Hats, Sunglasses<hr/>
      <li>Water:</li>
      Bottles, iodine, distiller, Filter, Coffee Filters, Bandannas<hr/>
      <li>Hygene:</li>
      Toothpaste, Toothbrushes, Floss, Mouthwash, Soap, Shampoo, Conditioner,
      Brush, Comb, Deoderant, Razor, Lotion, Q-tips, Nail Clippers, Gold Bond,
      Tinactin, Loofas<hr/>
      <li>Clothing:</li>
      Hats, Pants, Swimsuit, Socks, Underwear, Sunglasses, Coats, Rain Gear,
      Hiking Boots, Camo, Ski Mask, Gloves<hr/>
      <li>Tools:</li>
      Leatherman, Shovel, <hr/>
      </div>
    );
  }
}

export default Kit;
